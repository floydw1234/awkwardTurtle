...
class TestDatabaseSchema:
    ...
    def test_users_table_unique_constraint_username(self, test_schema_db):
        ...
        # Check if there's a unique constraint OR a unique index on username
        has_unique_constraint = any(
            "username" in str(c.get("column_names", [])) for c in constraints
        )
        has_unique_index = any(
            "username" in idx.get("column_names", []) and idx.get("unique")
            for idx in indexes
        )

        assert has_unique_constraint or has_unique_index, (
            "users table should have unique constraint or unique index on username"
        )
    ...
    def test_users_table_nullable_constraints(self, test_schema_db):
        ...
        assert column_map["username"]["nullable"] is False, (
            "username column should not be nullable"
        )
        assert column_map["hashed_password"]["nullable"] is False, (
            "hashed_password column should not be nullable"
        )
        assert column_map["is_active"]["nullable"] is True, (
            "is_active column should be nullable (has default)"
        )
        assert column_map["created_at"]["nullable"] is True, (
            "created_at column should be nullable (has default)"
        )
        assert column_map["updated_at"]["nullable"] is True, (
            "updated_at column should be nullable (has default)"
        )
