...
def pg_engine():
    ...
    try:
        ...
        pytest.fail(
            f"Cannot connect to PostgreSQL at {POSTGRES_HOST}:{POSTGRES_PORT}: {e}"
        )
    finally:
        engine.dispose()
...
class TestPostgreSQLConnection:
    ...
    def test_postgres_server_responds(self):
        ...
        try:
            ...
            pytest.fail(
                f"PostgreSQL server at {POSTGRES_HOST}:{POSTGRES_PORT} is not responding: {e}"
            )
    ...
    def test_awkward_turtle_database_exists(self, pg_engine):
        ...
        with pg_engine.connect() as conn:
            ...
            assert result.scalar() == 1, (
                "Should be able to execute query on awkward_turtle_db"
            )
...
class TestPostgreSQLSchema:
    ...
    def test_messages_table_exists(self, pg_engine):
        ...
        assert "messages" in tables, (
            "messages table should exist in PostgreSQL database"
        )
    ...
    def test_notifications_table_exists(self, pg_engine):
        ...
        assert "notifications" in tables, (
            "notifications table should exist in PostgreSQL database"
        )
    ...
    def test_friendships_table_exists(self, pg_engine):
        ...
        assert "friendships" in tables, (
            "friendships table should exist in PostgreSQL database"
        )
    ...
    def test_users_table_primary_key(self, pg_engine):
        ...
        assert pk["constrained_columns"] == ["id"], (
            "users primary key should be on 'id' column"
        )
    ...
    def test_messages_table_primary_key(self, pg_engine):
        ...
        assert pk["constrained_columns"] == ["id"], (
            "messages primary key should be on 'id' column"
        )
    ...
    def test_notifications_table_primary_key(self, pg_engine):
        ...
        assert pk["constrained_columns"] == ["id"], (
            "notifications primary key should be on 'id' column"
        )
    ...
    def test_users_table_index_username(self, pg_engine):
        ...
        assert "ix_users_username" in index_names, (
            "users table should have index on username"
        )
    ...
    def test_messages_table_foreign_keys(self, pg_engine):
        ...
        assert sender_fk_found, (
            "messages table should have foreign key from sender_id to users.id"
        )
        assert receiver_fk_found, (
            "messages table should have foreign key from receiver_id to users.id"
        )
    ...
    def test_notifications_table_foreign_key(self, pg_engine):
        ...
        assert user_fk_found, (
            "notifications table should have foreign key from user_id to users.id"
        )
    ...
    def test_friendships_table_columns(self, pg_engine):
        ...
        assert "user1_id" in column_names, (
            "friendships table should have user1_id column"
        )
        assert "user2_id" in column_names, (
            "friendships table should have user2_id column"
        )
    ...
    def test_friendships_table_primary_key_composite(self, pg_engine):
        ...
        assert len(pk_columns) == 2, (
            "friendships table should have composite primary key"
        )
        assert "user1_id" in pk_columns, (
            "friendships primary key should include user1_id"
        )
        assert "user2_id" in pk_columns, (
            "friendships primary key should include user2_id"
        )
    ...
    def test_friendships_table_foreign_keys(self, pg_engine):
        ...
        user1_fk_found = False
        user2_fk_found = False

        for constrained, referred in fk_names:
            if constrained == ["user1_id"] and referred == ["id"]:
                user1_fk_found = True
            if constrained == ["user2_id"] and referred == ["id"]:
                user2_fk_found = True

        assert user1_fk_found, (
            "friendships table should have foreign key from user1_id to users.id"
        )
        assert user2_fk_found, (
            "friendships table should have foreign key from user2_id to users.id"
        )
    ...
    def test_users_table_nullable_constraints(self, pg_engine):
        ...
        assert column_map["is_active"]["nullable"] is True, (
            "is_active should be nullable (has default)"
        )
        assert column_map["created_at"]["nullable"] is True, (
            "created_at should be nullable (has default)"
        )
        assert column_map["updated_at"]["nullable"] is True, (
            "updated_at should be nullable (has default)"
        )
    ...
    def test_messages_table_nullable_constraints(self, pg_engine):
        ...
        assert column_map["is_read"]["nullable"] is True, (
            "is_read should be nullable (has default)"
        )
        assert column_map["read_at"]["nullable"] is True, (
            "read_at should be nullable (can be None)"
        )
    ...
    def test_notifications_table_nullable_constraints(self, pg_engine):
        ...
        assert column_map["user_id"]["nullable"] is False, (
            "user_id should NOT be nullable"
        )
        assert column_map["notification_type"]["nullable"] is False, (
        ...
        assert column_map["related_id"]["nullable"] is True, (
            "related_id should be nullable"
        )
